...
homologous.groups.file <- '../data/dplex_homologous_groups_orthologs_only.json'
homologous.groups <- rjson::fromJSON(file=homologous.groups.file)

#load tau data
tau.data <- read.csv('../data/dpl_stage_specificity_data.csv')
...
#write to file
write.csv(homologus.group.data, file='../data/dplex_homologous_group_tau_var_orthogroup_only.csv', row.names=FALSE)
