...
#mean gene density
E_p = 0
for value in null_dict['E_biased']['n_clusters']:
    if value >= E_n_clusters:
        E_p += 1
null_count = len(null_dict['E_biased']['n_clusters'])
print(f"E: Full null, P(N>=O) ={E_p/1000}")
print(f'E: Non 0 null, P(N>=O) = {E_p/null_count}')

L1_p = 0
for value in null_dict['L1_biased']['n_clusters']:
    if value >= L1_n_clusters:
        L1_p += 1
null_count = len(null_dict['L1_biased']['n_clusters'])
print(f"L1: Full null, P(N>=O) ={L1_p/1000}")
print(f'L1: Non 0 null, P(N>=O) = {L1_p/null_count}')

L2_p = 0
for value in null_dict['L2_biased']['n_clusters']:
    if value >= L2_n_clusters:
        L2_p += 1
null_count = len(null_dict['L2_biased']['n_clusters'])
print(f"L2: Full null, P(N>=O) ={L2_p/1000}")
print(f'L2: Non 0 null, P(N>=O) = {L2_p/null_count}')

L3_p = 0
for value in null_dict['L3_biased']['n_clusters']:
    if value >= L3_n_clusters:
        L3_p += 1
null_count = len(null_dict['L3_biased']['n_clusters'])
print(f"L3: Full null, P(N>=O) ={L3_p/1000}")
print(f'L3: Non 0 null, P(N>=O) = {L3_p/null_count}')

D_p = 0
for value in null_dict['D_biased']['n_clusters']:
    if value >= D_n_clusters:
        D_p += 1
null_count = len(null_dict['D_biased']['n_clusters'])
print(f"D: Full null, P(N>=O) ={D_p/1000}")
print(f'D: Non 0 null, P(N>=O) = {D_p/null_count}')

L4_p = 0
for value in null_dict['L4_biased']['n_clusters']:
    if value >= L4_n_clusters:
        L4_p += 1
null_count = len(null_dict['L4_biased']['n_clusters'])
print(f"L4: Full null, P(N>=O) ={L4_p/1000}")
print(f'L4: Non 0 null, P(N>=O) = {L4_p/null_count}')

A_p = 0
for value in null_dict['A_biased']['n_clusters']:
    if value >= A_n_clusters:
        A_p += 1
null_count = len(null_dict['A_biased']['n_clusters'])
...
#L2: Full null, P(N>=O) =0.051
#L2: Non 0 null, P(N>=O) = 0.051
#L3: Full null, P(N>=O) =1.0
#L3: Non 0 null, P(N>=O) = 1.0
#D: Full null, P(N>=O) =0.047
#D: Non 0 null, P(N>=O) = 0.047
#L4: Full null, P(N>=O) =0.573
#L4: Non 0 null, P(N>=O) = 0.573
...
fig, [ax1,ax2,ax3,ax5,ax6,ax7] = plt.subplots(1, 6, figsize=(15, 3))
...
#Embryo biased
sns.kdeplot(null_dict['E_biased']['n_clusters'], ax=ax1, color=colors[0], 
            fill=True, alpha=0.5, clip=(0, None), common_norm=True)
ax1.axvline(x=E_n_clusters, color=colors[0], linestyle='--', linewidth=2)

#L1 biased
sns.kdeplot(null_dict['L1_biased']['n_clusters'], ax=ax2, color=colors[1], 
            fill=True, alpha=0.5, clip=(0, None), common_norm=True)
ax2.axvline(x=L1_n_clusters, color=colors[1], linestyle='--', linewidth=2)

#L2 biased
sns.kdeplot(null_dict['L2_biased']['n_clusters'], ax=ax3, color=colors[2], 
            fill=True, alpha=0.5, clip=(0, None), common_norm=True)
ax3.axvline(x=L2_n_clusters, color=colors[2], linestyle='--', linewidth=2)
...
#D biased
sns.kdeplot(null_dict['D_biased']['n_clusters'], ax=ax5, color=colors[4], 
            fill=True, alpha=0.5, clip=(0, None), common_norm=True)
ax5.axvline(x=D_n_clusters, color=colors[4], linestyle='--', linewidth=2)

#L4 biased
sns.kdeplot(null_dict['L4_biased']['n_clusters'], ax=ax6, color=colors[5], 
            fill=True, alpha=0.5, clip=(0, None), common_norm=True)
ax6.axvline(x=L4_n_clusters, color=colors[5], linestyle='--', linewidth=2)

#A biased
sns.kdeplot(null_dict['A_biased']['n_clusters'], ax=ax7, color=colors[6], 
            fill=True, alpha=0.5, clip=(0, None), common_norm=True)
ax7.axvline(x=A_n_clusters, color=colors[6], linestyle='--', linewidth=2)
...
ax3.set_title('L2: P(N$\geq$O) = 0.051', fontsize=14, multialignment='center')
ax5.set_title('Dauer: P(N$\geq$O) = 0.047', fontsize=14, multialignment='center')
ax6.set_title('L4: P(N$\geq$O) = 0.573', fontsize=14, multialignment='center')
ax7.set_title('Adult: P(N$\geq$O) < 0.001', fontsize=14, multialignment='center')

plt.tight_layout()
plt.savefig('../figures/cluster_regions_frequencies_null_vs_obs.pdf', bbox_inches='tight')
